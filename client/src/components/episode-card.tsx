    ...
    <div className="flex items-center gap-2 p-2 rounded-lg hover:bg-gray-800 transition-colors">
      <div className="relative flex-shrink-0">
        ...
        <div className="absolute inset-0 bg-black/30 rounded flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity">
          <button 
            ...
            className="w-6 h-6 sm:w-8 sm:h-8 bg-primary hover:bg-primary/90 rounded-full transition-colors flex items-center justify-center"
          >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-baseline gap-2">
          <span className="text-xs font-medium text-primary">Ép. {episode.episode_number}</span>
          <span className="text-xs text-gray-400">
            {episode.air_date ? new Date(episode.air_date).toLocaleDateString('fr-FR') : ''}
          </span>
        </div>
        <h3 className="font-medium text-sm line-clamp-1 mt-1">{episode.name}</h3>
      ...
      <a 
        ...
        className="flex-shrink-0 w-8 h-8 sm:w-10 sm:h-10 bg-primary hover:bg-primary/90 rounded-full transition-colors flex items-center justify-center"
      >
